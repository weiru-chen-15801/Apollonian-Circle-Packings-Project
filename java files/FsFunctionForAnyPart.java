...
public class FsFunctionForAnyPart {
	...
	public static int getNumberOfPoints(double newS, int newM) {
		int M = newM;
		...
		for (int i = 0; i < totalList.length; i++) {
			if (totalList[i] != null){
				double x = totalList[i].getCenter().getRez();
				double y = totalList[i].getCenter().getImz();
				double distance = Math.sqrt(x*x+y*y);
				if (distance < (double) newS/newM)
					numberOfPoints++;
			}
		}
...
public static double getProb(double newS, int newM) {
        ...
        double prob = number / (double) (2*getNumberOfPoints(newS, M));
