...
class KDTree
{
    ...
    public KDNode find_nearest(double[] x)
    {
        ...
        ;
        //d_min should not be 0 since cannot be itself
        if (parent.equal(x, parent.x, 2) == true && (d_min != 0))
    ...
    //is it correct
    public double nearestDistance(double[] y) {
    	
    	KDNode a = find_nearest(y);
    	double[] z = a.x;
    	double distance = (double) Math.sqrt((z[0]-y[0])*(z[0]-y[0]) + 
    			(z[1]-y[1])*(z[1]-y[1]));
    	return distance;
