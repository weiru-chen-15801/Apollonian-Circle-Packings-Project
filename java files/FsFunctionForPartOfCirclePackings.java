...
public class FsFunctionForPartOfCirclePackings {
	...
	public static int getNumberOfPoints(int newM) {
		...
		for (int i = 0; i < totalList.length; i++) {
			if (totalList[i] != null){
				if (totalList[i].getCenter().getRez() >= 0)
					numberOfPoints++;
			}
		}
...
public static double getProb(double newS, int newM) {
        ...
        //2*getNumberOfPoints or just getNumberOfPoints
 
        double prob = number / (double) (2*getNumberOfPoints(M));
