...
public class ElectronicEnergyDoubleI {
...
public static double getSigma(double newS, int newM) {
        	...
        	if (totalList[i] != null) {
        		for (int j = 0; j < totalList.length; j++) {
        			if (totalList[j] != null && i != j) {
        				...
        				temp = ElectrostaticEnergy.value(zi, zj);
        				//divide or times
        ...
        double e_M = sum / (double) (Math.pow(M, ro));
