...
public class GsFunction {
	public static boolean checkInArray(Point2D.Double point, Point2D.Double[] checkArray) {
		for (int i = 0; i < checkArray.length; i++) {
			if (point.equals(checkArray[i]))
				return true;
		}
		return false;
	}
	
	public static double getProb1(int newM) {
       ...
       //what is the size of kdt
        ...
        KDTree kdt = new KDTree(numpoints*100);
        ...
        for (int i = 1; i < circlesList1.length; i++) {
        	if (circlesList1[i] != null) {
                ...
                if ((checkInArray(temp, checkList) == false) && 
                		(checkInArray(temp2, checkList) == false)) {
                	if ((distance/(double) minRadius) <= s) {
                		lessThanS++;
        ...
        double prob = lessThanS / (double) (Math.pow(M, ro));
...
public static double getProb2(int newM) {
...
public static double getProb3(int newM) {
	
	double theta1 = 1.5724;
	double theta2 = 5.0126;
	double ro = 1.305688;
